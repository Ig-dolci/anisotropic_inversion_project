...
\title{\textbf{Projeto de Pesquisa de Pós-Doutorado}\\[0.5cm]
       \Large{Inversão de Forma de Onda Completa Anisotrópica Usando Método de Elementos Finitos} \\[0.5cm]
       \large{\textit{Anisotropic Full Waveform Inversion Using Finite Element Method}}}
\author{
    \textbf{Daiane Iglesia Dolci}\\
    \textit{daia.dolci@gmail.com}\\
    Currículo Lattes: hhttp://lattes.cnpq.br/1341115679564843\\[0.5cm]
...
\begin{abstract}
Este projeto de pesquisa propõe o desenvolvimento de um \textit{framework} computacional para a Inversão de Forma de Onda Completa (FWI) em meios anisotrópicos, utilizando o Método de Elementos
Finitos de alta ordem. O foco principal é a integração de
diferenciação algorítmica para o cálculo eficiente de
gradientes e paralelismo, visando aplicações em modelos geofísicos complexos.

\vspace{0.5cm}
\noindent \textbf{Palavras-chaves:} Inversão de Forma de Onda Completa (FWI); Diferenciação Automática; Anisotropia.
\end{abstract}
...
diferença entre dados sísmicos observados e simulados \cite{Tarantola:1984, Virieux2009}.
...
o leitor pode consultar os trabalhos de \cite{Virieux2009, Operto2013, AlkhalifahPlessix2014} que apresentam
...
ou isotrópico em regiões em essência elásticas e anisotrópicas pode resultar em erros de imageamento \cite{AlkhalifahPlessix2014, AkiRichards2002}.
De acordo com \cite{Thomsen1986}, as causas de anisotropia em rochas, nesse caso superfícies rochosas, são  por exemplo quando há fraturas presentes nas rochas, sequência de finas camadas isotrópicas ou anisotrópicas (isto é, estratificação
fina em relação ao comprimento de onda sísmica), alinhamento preferencial de minerais, ou combinação desses fatores. Quando estruturas sedimentares
estratificadas são deformadas por tectonismo regional ou por forças locais (por exemplo, a intrusão de corpos de sal),
o eixo de simetria do meio é rotacionado. Essa inclinação dá origem à chamada Isotropia
Transversal Inclinada (TTI, do inglês \textit{Tilted Transverse Isotropy}), um modelo físico indispensável
para descrever corretamente a propagação de ondas em flancos de domos salinos e bacias estruturalmente complexas \cite{Tsvankin2012}.
...
Isso torna a inversão desafiadora, exigindo estratégias de parametrização robustas para mitigar tais ambiguidades.

Métodos baseados em Diferenças Finitas (FDM) são amplamente utilizados devido à sua eficiência computacional
e facilidade de implementação. Entretanto, o FDM
apresenta limitações na representação de geometrias complexas, dada a
rigidez inerente às malhas estruturadas \cite{Virieux2011}. Por outro lado, o Método de Elementos Finitos (FEM) oferece maior flexibilidade geométrica
...
aplicabilidade de elementos espectrais para a propagação de ondas em meios heterogêneos. Trabalhos
...
Além disso, Roberts et al. \cite{roberts2022spyro} implementaram solucionadores de elementos finitos de alta ordem triangulares (2D) e
...
proposta está na integração de Diferenciação Algorítmica (AD) via \texttt{pyadjoint} \cite{Mitusch2019} para o cálculo automatizado de gradientes,
aliado a estratégias ótimas de \textit{checkpointing}, funcional objetivo reduzido com paralelismo de fontes de onda e um \textit{framework} computacional
flexível para estratégias robustas de inversão multiparamétrica.

\section{Justificativa}

O desenvolvimento de um solucionador baseado no Método de Elementos Finitos (FEM) de alta ordem, 
com AD e gestão eficiente de memória é estratégico para o avanço do FWI com sistemas de equações complexas, como a anisotropia TTI. 
Embora ferramentas de FEM ofereçam precisão superior em malhas não estruturadas, a complexidade de implementação de códigos adjuntos (necessários para o cálculo de gradientes)
pode limitar a pesquisa de novas físicas e estratégias de inversão, como às exigidas para inversão com anisotropia TTI.

A utilização de AD para automatizar a obtenção das equações adjuntas elimina erros de derivação manual e acelera o desenvolvimento. Contudo, o uso de AD em problemas 
com sistema de equações em grandes escalas (como em FWI 3D) apresenta desafios significativos de desempenho e consumo de memória. Investigar o
equilíbrio entre a sobrecarga da AD e a eficiência computacional, comparando-a com abordagens manuais, é também um objetivo deste projeto.
...
com AD as diferentes estratégias de parametrização e regularização para mitigar ambiguidades. Portanto, a ferramenta proposta
tem como foco ser uma plataforma para desenvolvimentos metodológicos em inversão sísmica.
...
O objetivo geral é implementar e verificar um \textit{framework} computacional para FWI 3D TTI baseado em elementos finitos de alta ordem,
integrando AD para o cálculo automatizado de gradientes e paralelismo \textit{ensemble} para o processamento eficiente de
...
industriais 3D, implementando uma arquitetura flexível com AD e estratégias para mitigar o \textit{trade-off} entre os parâmetros físicos.
...
habilitam o Firedrake aos cálculos automatizados de gradientes baseados no método adjunto sem necessidade de derivação manual das equações.
...
exclusivamente a linguagem UFL para resolver as equações. Esta característica é essencial para obter
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    ...
    \fill[blue,rounded corners=2mm,even odd rule,opacity=0.25]  (2.2,-6.0) rectangle (5.85,-3.5);
    \fill[blue,rounded corners=2mm,even odd rule,opacity=0.25]  (6.2,-6.0) rectangle (9.85,-3.5);
    \fill[blue,rounded corners=2mm,even odd rule,opacity=0.25]  (10.2,-6.0) rectangle (13.85,-3.5);
    \fill[red,rounded corners=2mm,even odd rule,opacity=0.25]  (2.2,-8.5) rectangle (5.85,-6.4);
    \fill[red,rounded corners=2mm,even odd rule,opacity=0.25]  (6.2,-8.5) rectangle (9.85, -6.4);
    \fill[red,rounded corners=2mm,even odd rule,opacity=0.25]  (10.2,-8.5) rectangle (13.85, -6.4);
    % Arrows
...
resolver a EDP direta e armazenar as variáveis de estado diretas,
...
re-executada a partir do último estado armazenado disponível até o passo adjunto atual. Isso permite que um número menor de estados diretos
sejam armazenados, às custas de um maior custo computacional de tempo, já que passos diretos são executados mais de uma vez.
...
Define-se o resíduo de primeira ordem:
\begin{equation}
